...
function update_sum(){
     ...
     for(var i=0; i<sum.length; i++){
          sum[i].innerHTML = " _ "+expense_per_category[i]+" Rs.spent";
     }
}
function add_expense(){
    ...
    if(amount && date && note && selected && selected!='selection'){
        ...
        expense_per_category[category-1]=parseInt(expense_per_category[category])+parseInt(amount);
        update_sum();
        console.log(count);
 ...
 function add_category(){
    ...
    if(new_category_value){
        ...
        new_category_span.innerHTML= `<span>${new_category_value}<span class="sum">_${expense_per_category[count]} Rs.spent</span></span>` ;
